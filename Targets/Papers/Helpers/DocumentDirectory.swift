...
struct DocumentDirectory {
    ...
    func write<T: Encodable>(
            ...
            let nestedFolderURL = url.appendingPathComponent("MyAppFiles")
    ...
    func read(from filename: String) -> Data? {
        do {
            ...
            let data = try Data.read(from: fileURL)
    ...
    func findPapers() -> [URL] {
        var paperURLs: [URL] = []
        do {
            let directoryContents = try FileManager.default.contentsOfDirectory(at: url.appendingPathComponent("MyAppFiles"), includingPropertiesForKeys: nil)
