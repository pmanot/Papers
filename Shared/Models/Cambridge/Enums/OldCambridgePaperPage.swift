...
struct OldCambridgePaperPage: Codable, Hashable {
...
enum CambridgePageType: Codable, Hashable {
    ...
    case datasheet
    case markSchemePage
    case multipleChoicePage(indexes: [OldQuestionIndex])
    case questionPaperPage(index: OldQuestionIndex)
    ...
    init(from decoder: Decoder) throws { // conform to Decodable
        let container = try decoder.container(keyedBy: CodingKeys.self)
                ...
                case "markScheme":
                    self = .markSchemePage
                case "question":
                    let index = try container.decode(OldQuestionIndex.self, forKey: .associatedValue)
                    self = .questionPaperPage(index: index)
                case "multipleChoiceQuestions":
                    let indexes = try container.decode([OldQuestionIndex].self, forKey: .associatedValue)
